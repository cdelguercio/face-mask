...
class Calibration:
    ...
    def get_projector_frame(self) -> Optional[np.ndarray]:
        """Return a full-resolution BGRA grid image for Spout output, or None if not calibrating."""
        if not self.active:
            return None
    ...
    def _save(self):
        ...
        data = {
            "pairs": [
                {
                    "camera": list(p.camera_pt),
                    "projector": list(p.projector_pt),
                }
                for p in self.pairs
            ],
    ...
    def _load(self):
        ...
        try:
            ...
            self.pairs = [
                PointPair(
                    camera_pt=tuple(p["camera"]),
                    projector_pt=tuple(p["projector"]),
                )
            ...
            self._recompute()
            if self.homography is not None:
                print(f"[Calibration] Loaded {len(self.pairs)} point pairs from disk.")
            elif self.pairs:
                print(f"[Calibration] Loaded {len(self.pairs)} pairs (need 4+ for homography).")
